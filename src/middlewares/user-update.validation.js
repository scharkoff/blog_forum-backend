...
export function userUpdateValidation(req, res, next) {
    ...
    if (req.body.fullName) {
        return Promise.all(fullNameValidation.map(rule => rule.run(req)))
            .finally(() => {
                next();
            });
    }
    ...
    if (req.body.email) {
        return Promise.all(emailValidation.map(rule => rule.run(req)))
            .finally(() => {
                next();
            });
    }
    ...
    if (req.body.password) {
        return Promise.all(passwordValidation.map(rule => rule.run(req)))
            .finally(() => {
                next();
            });
    }
    ...
    if (req.body.avatarUrl) {
        return Promise.all(avatarValidation.map(rule => rule.run(req)))
            .finally(() => {
                next();
            });
    }
